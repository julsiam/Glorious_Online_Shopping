...
<body>
    <div class="app-container" style="display: grid;">
        <!-- <div class="sidebar">
            <div class="sidebar-header">
                <div class="app-icon" style="display: flex;align-items: center;justify-content: center;gap:5px">
        ...
        <div class="app-content">
            ...
            <h1 style="color: white;text-align: center;">Cart List</h1>
                ...
                <div class="app-content-actions-wrapper">
                    <div class="filter-button-wrapper">
                        ...
                        <div class="filter-menu">
                            ...
                            <select style="cursor: pointer;color:white;background-color: black;">
                                <option style="cursor: pointer;">All Categories</option>
            ...
            <div class="my-cart-list">
                ...
                <div class="my-cart-body">
                    ...
                    $name = $mysqli->query("SELECT cart.id, product_details.product_name,product_details.product_image, product_details.product_info, 
                    product_details.price, cart.quantity, cart.subtotal FROM `cart` 
                    INNER JOIN `product_details` ON product_details.id = cart.product_id
                    INNER JOIN `profile_information` ON profile_information.id = cart.user_id
                    WHERE profile_information.id = '$id'");
                    foreach ($name as $info) { ?>

                        <form action="add-process.php" method="post" class="my-item-details">
                            <input name="cart-id" type="text" value="<?= $info['id'] ?>" style="display: none;">

                            <div class="fourty">

                                <a href="#"><img src="test/uploads/<?= $info['product_image'] ?>" alt="phone" class="my-item-image"></a>
                                <div style="text-align: center;">
                                    <p style="font-size: 20px;"><?= $info['product_name'] ?></p>
                                    <small style="position: relative;top:-15px;font-weight: normal;font-size: 12px;"><?= $info['product_info'] ?></small>
                                </div>

                            </div>
                            <div class="twenty ">Php. <?= $info['price'] ?></div>
                            <input type="text" name="initial-price" class="initial-price" value="<?= $info['price'] ?>" style="display: none;">
                            <div class="twenty">
                                <button class="add-cart" style="color:black">+</button>
                                <input name="input-quantity" style="color:black" class="twenty input-quantity" type="text" value="<?= $info['quantity'] ?>" />
                                <button class="minus-cart" style="color:black">-</button><br>
                                <small class="piece-count"><?= $info['quantity'] ?> piece/s</small>
                            </div>
                            <div class="twenty ">
                                <input type="text" class="initial-subtotal" value="<?= $info['subtotal'] ?>" style="display: none;">

                                <span name="price-value" class="price-value"> Php. <?= $info['subtotal'] ?></span>


                            </div>
                            <div class="twenty" style="width:100%;text-align: right;">

                                <button type="submit" name="save_primary" class="remove-item-button save_primary" style="background-color: green;padding:8px 20px 8px 20px">Save</button>
                                <button class="remove-item-button cancel-primary" style="background-color: #bb2124;padding:8px 20px 8px 20px">Cancel</button>
                                <div class="delete-pop" style="display:none;position: absolute;background-color: white;padding:10px 20px 10px 20px;border-radius: 4px;right:50px;bottom:cal(100% );text-align: center;
                             ">
                                    <p style="font-size: 13px;color:black;font-weight: bold;">Are you sure do you want to cancel your payment!</p>
                                    <button class="cancel-btn" style="border-color: transparent;font-weight: normal;cursor: pointer;background-color: #2869ff;color:white">Cancel</button>
                                    <button name="p-delete-btn" class="p-delete-btn" style="border-color: transparent;font-weight: normal;cursor: pointer;background-color: #2869ff;color:white">Confirm</button>
                                </div>
                ...
                <div class="checkout-footer" style="margin-top: 30px;text-align: right;padding:30px">
                    <div style="padding:10px; border-radius: 4px;text-align: right;">
                        <input type="text" placeholder="Discount Code" style="padding:10px;width:400px;">
                        <button type="submit" style="padding:10px 20px 10px 20px;font-weight: bold;background: black;color:white">Apply</button>
                    </div>
                    <div style="text-align: right;color:white">
                        <small><b style="color:lightgray;font-weight: normal;">Overall Total</b>: Php. 12000</small>
                    </div>
                    <a> <button style=" padding: 10px;width:498px;font-weight: bold;background-color: #2869ff ;color:white;
                    border-color: transparent;border-radius: 4px;font-size: 18px;">Proceed to Checkout</button></a>
    ...
    <script>
        ...
        document.querySelectorAll('.cancel-primary').forEach((btn) => {
            btn.addEventListener('click', function(event) {
                event.preventDefault();
                btn.parentElement.querySelector('.delete-pop').style.display = "block";
        ...
        document.querySelectorAll('.my-item-details').forEach((item) => {
            item.querySelector('.add-cart').addEventListener('click', function(event) {
                event.preventDefault();
                ...
                let quan = item.querySelector('.initial-price').value;
            ...
            item.querySelector('.minus-cart').addEventListener('click', function(event) {
                event.preventDefault();
                let quantity = item.querySelector('.input-quantity').value;
